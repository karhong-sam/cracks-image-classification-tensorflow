# -*- coding: utf-8 -*-
"""
Created on Mon Jul 20 01:12:36 2020
...
root.title("Crack Image Classifier")
...
def open_file(initialdir='/'):
    ...
    file_path  = askopenfilename(initialdir=initialdir, filetypes = [ ('Saved Model', '*.h5' ) ]  )
    dialog_var.set("Browse Models but not Yet Load it")
    h5_var.set(file_path)

    return file_path

def load_weights():
    dialog_var.set("Loading model's weight.......")
...
def open_image(initialdir='/'):
    ...
    image = image.resize((227,227)) # resize image to 32x32
...
def test_image():
    ...
    # predict
    old = time.time()
    predictions = model.predict_classes(imgs)[0]
    new = time.time()
    print(predictions,results[predictions])
    print(new-old)
    result_text = "output class: "+str(results[predictions])
...
dialog_var.set("Welcome to the World of Deep Learning!")
